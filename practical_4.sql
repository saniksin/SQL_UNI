...
-- Висновки (Тема 4)
-- ================================================================
-- У ході виконання практичної роботи було спроєктовано та реалізовано
-- структуру бази даних publishing засобами мови SQL. На етапі DDL
-- створено 8 таблиць (5 базових та 3 асоціативних), визначено первинні
-- й зовнішні ключі, обмеження UNIQUE на ISBN та Email, CHECK на
-- кількість і ціну в OrderItem, а також правила каскадного видалення
-- (ON DELETE CASCADE для асоціативних таблиць та ON DELETE RESTRICT
-- для Contracts).
--
-- На етапі DML у кожну таблицю внесено по 10 тестових записів.
-- Для асоціативних таблиць та Contracts / OrderItem використано
-- конструкцію INSERT ... SELECT із пошуком батьківського рядка за
-- унікальним полем (Email, ISBN, ClientName + OrderDate) — це зробило
-- скрипт незалежним від порядку AUTO_INCREMENT і безпечним для
-- повторних запусків. Задачі 4 і 5 (SELECT * та COUNT через UNION ALL)
-- підтвердили, що в усіх таблицях зберігається очікувана кількість
-- рядків (по 10), а зв'язки між ними коректні.
--
-- Додатково продемонстровано команди UPDATE (зміна статусу замовлення,
-- року видання книги, закриття контракту) та DELETE зі спрацьовуванням
-- каскадного видалення пов'язаної позиції OrderItem. Отриманий результат
-- показав, що декларативно описані обмеження цілісності працюють
-- коректно, а база готова до виконання DQL-запитів у практичній 5.
